...
`create_scalar_index()` - Create scalar indices from the lance-ray API. Inverted/FTS/BTREE use Ray distributed building. ZONEMAP is supported through Lance's native scalar index builder because the current pylance distributed metadata merge API does not expose ZONEMAP merge support yet. Will add more distributed index type support in the future.
...
**Note:** For distributed scalar indexing, currently only `"INVERTED"`, `"FTS"` and `"BTREE"` index types are supported. `"ZONEMAP"` creation is available through the same API, but it uses Lance's native full-dataset scalar index builder until pylance exposes distributed ZONEMAP metadata merge support.
...
- **Index Type Support**: For distributed indexing, currently only `"INVERTED"`/`"FTS"`/`"BTREE"` index types are supported. `"ZONEMAP"` creation is available through the same API, but it uses Lance's native full-dataset scalar index builder until pylance exposes distributed ZONEMAP metadata merge support.
